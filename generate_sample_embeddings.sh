...
CACHE_DIR="model_cache"  # local model cache (EmbeddingService defaults to /app, unwritable off-container)
# ─────────────────────────────────────────────────────────────────────────────

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "${ROOT_DIR}"

# EmbeddingService reads EMBEDDING_CACHE_PATH; point it at the repo-local cache
# so model files download to a writable location instead of /app/model_cache.
...
echo "  Input        : ${INPUT}"
echo "  Output       : ${OUTPUT}"
echo "  Dimensions   : ${DIMENSIONS}  (PyTorch + EmbeddingGemma-300M, in-process)"
